 ...
 * @returns ReactPortal
 */
const ModalPortal = ({ display, element, classCustom, styleCustom }) => {
        ...
        return () => {
            ...
            if (modalRootEl.firstChild && modalRootEl.firstChild === elRef.current) {
                try {
                    ...
                    showDialogFailed("", "Si � verificato un errore, si prega di ricaricare la pagina");
                    console.error("Si � verificato un errore, si prega di ricaricare la pagina");
                }
            }
        };
    }, []);
    return createPortal(_jsxs("div", { className: `ModalsPortal__wrapper ${display ? "open" : "closed"} ${classCustom}`, children: [_jsx("div", { className: "ModalsPortal__backdrop", children: " s" }), _jsx("div", { className: "ModalsPortal__container container p-5", style: styleCustom, children: element })] }), elRef.current);
};
/**
 * Default export
 */
